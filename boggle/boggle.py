...
def dfs(board, x ,y, idx, word):
    # if out of bounds or incorrect char, return and mark as seen

    if x < 0 or y < 0 or y > 4 or x > 4 or board[x][y] != word[idx]:
        return 0
...
def find(board, word):
    ...
    for i in range(rows):
        for j in range(cols):
            print(dfs(board, i, j, idx, word))
            if dfs(board, i, j, idx, word) == True:
                print('truuuu')
                return True
    return False
